...
def generate_launch_description():
    ...
    launch_rviz_arg = DeclareLaunchArgument(
        'launch_rviz',
        default_value='false',
        description='Jalankan RViz2 (true/false). Default: false.'
    )
    ...
    # Node MAVROS
    # ==========================================================
    custom_config_path = os.path.join(package_dir, 'config', 'apm_config.yaml')
    mavros_config_file = custom_config_path if os.path.exists(custom_config_path) else os.path.join(get_package_share_directory('mavros'), 'launch', 'apm_config.yaml')
    # Use custom pluginlists (with distance_sensor disabled) if available, else system default
    custom_pluginlists_path = os.path.join(package_dir, 'config', 'apm_pluginlists.yaml')
    mavros_pluginlists_file = custom_pluginlists_path if os.path.exists(custom_pluginlists_path) else os.path.join(get_package_share_directory('mavros'), 'launch', 'apm_pluginlists.yaml')
    ...
    gcs_bridge_node = Node(
        ...
        executable='real_gcs_bridge_node',
