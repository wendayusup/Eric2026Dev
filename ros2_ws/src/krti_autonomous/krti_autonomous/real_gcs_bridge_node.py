...
def is_valid_range(msg, min_dist=0.01, max_dist=50.0):
    if msg is None:
        return False
    dist = msg.range
...
class ROS2GCSBridgeNode(Node):
    def __init__(self):
        super().__init__('real_krti_gcs_bridge_node')
        ...
        # PositionTarget for direct, reliable altitude and yaw control in GUIDED mode
        ...
        # RasPi traffic light permission state
        self.waiting_for_permission = False   # True while holding at a WP for RasPi green light
        self.permission_wp_index = -1         # Index of WP currently waiting for permission
        self._permission_request_t = None     # Timestamp when permission was first requested

        # Navigation precision state
        self._last_wp_advance_t = 0.0         # Debounce: prevent re-triggering arrival within 2s
        self.segment_start_pos = None         # Exact ENU position of leg start (for tank turn hold)
        self.prev_target_position = None      # ENU of previous WP (for segment anchoring)
    ...
    def _mavlink_distance_listener(self):
        ...
        while rclpy.ok():
            try:
                if mav_conn is None:
                    mav_conn = mavutil.mavlink_connection('udp:127.0.0.1:14550', input=True)
    ...
    def local_pos_callback(self, msg):
        ...
        # Update relative altitude from EKF Local Position Z (0.00m at home/ground)
        if not (math.isnan(self.local_z) or math.isinf(self.local_z)):
            filtered_alt = self.filt_alt.filter(self.local_z)
    ...
    def _update_alt_avg(self):
        ...
        # If rangefinders are publishing valid positive readings, prioritize rangefinder
        if rf_f > 0.0 and rf_r > 0.0:
            raw_avg = (rf_f + rf_r) / 2.0
            filtered_alt = self.filt_alt.filter(raw_avg)
            current_telemetry['alt'] = round(filtered_alt, 2)
        elif rf_r > 0.0:
            filtered_alt = self.filt_alt.filter(rf_r)
            current_telemetry['alt'] = round(filtered_alt, 2)
        elif rf_f > 0.0:
            filtered_alt = self.filt_alt.filter(rf_f)
            current_telemetry['alt'] = round(filtered_alt, 2)
        elif self.local_z != 0.0:
            # Fallback to Local Position EKF altitude if no rangefinder available
            filtered_alt = self.filt_alt.filter(self.local_z)
            current_telemetry['alt'] = round(filtered_alt, 2)
    ...
    def rangefinder_callback(self, msg):
        ...
        # NOTE: RANGE F updates alt_front for display/logging only.
        # ALT used for navigation comes exclusively from ALT R via altitudefinder_callback.
        current_telemetry['connected'] = True
        self.last_any_data_time = time.time()
        self.last_rangefinder_time = time.time()
    ...
    def front_cam_callback(self, msg):
        ...
        # [FIX] Jangan timpa frame dari Pi (kamera asli) dengan frame simulasi ROS2.
        # Keduanya menulis ke key 'front' yang sama di latest_frames — kalau ada
        # node ROS2 lain yang masih publish ke /camera/front (sisa testing sim),
        # ini akan race dengan feed asli dari Raspberry Pi.
        if latest_camera_status.get('front', {}).get('connected'):
            return
        now = time.time()
    ...
    def bottom_cam_callback(self, msg):
        ...
        # [FIX] Sama seperti front — Pi selalu prioritas kalau sedang aktif konek.
        if latest_camera_status.get('bottom', {}).get('connected'):
            return
        now = time.time()
    ...
    def arm(self, state=True):
        ...
        def arm_done_cb(fut):
            try:
                res = fut.result()
                if res.success:
                    self.get_logger().info(f"[GCS] Arming state set to {state} SUCCESS!")
                    socketio.emit('upload_status', {'status': 'success', 'message': f'Drone {"ARMED" if state else "DISARMED"} successfully!'})
                else:
                    self.get_logger().warn(f"[GCS] Arming state set to {state} FAILED (Result code {res.result})")
                    socketio.emit('status_msg', {'type': 'error', 'text': f'[APM] Arming Failed (Code {res.result}). Check Safety Switch / PreArm checks!'})
            except Exception as e:
                self.get_logger().error(f"[GCS] Arming service exception: {e}")
        future.add_done_callback(arm_done_cb)
    ...
    def guided_timer_callback(self):
        ...
        # now_t defined FIRST — used in both position setpoint publish and movement logic below.
        ...
        if self.pending_takeoff_alt is not None:
            if current_mode == 'GUIDED':
                ...
                if now_ms - self._last_mode_req_time > 500:
                    self._last_mode_req_time = now_ms
                    self.get_logger().info(f"[GCS] Setting mode to GUIDED for takeoff (current: {current_mode})...")
                    self.set_mode("GUIDED")
                return
        ...
        # ENU yaw from local_position/pose — same frame as target_yaw and setpoint orientation.
        # NEVER use current_telemetry['yaw'] here; that value is negated for compass display.
        ...
        if getattr(self, 'is_climbing_to_wp_alt', False):
            ...
            if alt_err < 0.35 or alt_timeout:
                ...
                if self.segment_start_pos is not None:
        ...
        # PHASE 3: FORWARD TRANSIT ALONG PATH
            ...
            if getattr(self, 'auto_mission_active', False):
                ...
                if not hasattr(self, '_wp_leg_start_t') or self._wp_leg_start_t is None:
                    self._wp_leg_start_t = now_t

                # Debounce: skip arrival check within 1.5s of last advance
                if (now_t - self._last_wp_advance_t) >= 1.5:
                    alt_reached = (abs(real_dz) < 0.5)
                    pos_reached = (real_dist_2d < 0.8)
                    safety_timeout = (now_t - self._wp_leg_start_t) >= 60.0

                    if wp_type == 'landing':
                        is_reached = (real_dist_2d < 0.8 and alt_reached) or safety_timeout
                    elif wp_type == 'takeoff':
                        is_reached = False  # Handled in takeoff completion block
                    else:
                        is_reached = (pos_reached and alt_reached) or safety_timeout

                    if is_reached:
                        self._last_wp_advance_t = now_t
                        self._wp_leg_start_t = now_t
                        self.get_logger().info(
                            f"[GCS] WP {self.current_auto_wp_index} REACHED! "
                            f"(pos={real_dist_2d:.2f}m, alt_err={real_dz:.2f}m)"
                        )
                        current_wp = self.auto_mission_array[self.current_auto_wp_index]

                        # If this WP has an explicit target_heading, rotate to it NOW on arrival
                        ...
                        if payload_cmd == 'ON' or payload_cmd == 'SERVO_8S':
                            self.get_logger().info("[GCS] Payload Active! Opening servo (8s)...")
                            ...
                            def close_servo_task():
                                time.sleep(8)
                                self.get_logger().info("[GCS] 8s done. Closing servo...")
                        ...
                        if wp_type == 'landing':
                            self.get_logger().info("[GCS] Landing WP reached. Executing Auto Land.")
                            self.guided_active = False
                            self.land()
                            self.auto_mission_active = False
                        elif self.waiting_for_permission:
                            # Hold at this WP — wait for RasPi green light
                            if self._permission_request_t is None:
                                self._permission_request_t = now_t
                                self.permission_wp_index = self.current_auto_wp_index
                                self.get_logger().info(
                                    f"[GCS] Holding at WP {self.current_auto_wp_index} "
                                    "— waiting for RasPi traffic light (timeout 15s)"
                                )
                                socketio.emit('pi_permission_request', {
                                    'wp': self.current_auto_wp_index,
                                    'type': wp_type
                                })
                                socketio.emit('status_msg', {
                                    'type': 'warning',
                                    'text': f'[MISSION] Waiting for green light at WP {self.current_auto_wp_index + 1}'
                                })
                            elif (now_t - self._permission_request_t) > 15.0:
                                self.get_logger().warn(
                                    f"[GCS] Permission timeout at WP {self.current_auto_wp_index}! Auto-advancing."
                                )
                                socketio.emit('status_msg', {
                                    'type': 'warning',
                                    'text': '[MISSION] Permission timeout — auto-advancing!'
                                })
                                self.waiting_for_permission = False
                                self._permission_request_t = None
                                self.current_auto_wp_index += 1
                                self.start_next_auto_waypoint()
                        else:
                            self.current_auto_wp_index += 1
        ...
        if self.use_velocity_control:
            ...
            if now_t - self._last_pub_t >= 0.05:  # 20Hz for smooth, precise trajectory
    ...
    def start_next_auto_waypoint(self):
        ...
        # Clear permission state for incoming WP
        self.waiting_for_permission = False
        self._permission_request_t = None

        # Skip takeoff if drone is already airborne
        is_on_ground = self.local_z < 0.5
        if wp_type == 'takeoff' and not is_on_ground:
            self.get_logger().info("[GCS] Drone sudah di udara. Melewati Takeoff.")
            self.current_auto_wp_index += 1
            if self.current_auto_wp_index < len(self.auto_mission_array):
                self.start_next_auto_waypoint()
            else:
        ...
        try:
            ...
            self.get_logger().error(f"[GCS] Error parsing WP data: {err}")
        ...
        if wp_type == 'landing':
            if speed <= 0.1:
                speed = 2.5
            # For landing WP, fly to landing coordinate at previous WP cruise altitude, then auto-land
            z = self.prev_target_position[2] if self.prev_target_position is not None else alt_rel
        else:
            z = alt_rel

        # Dynamic cruise speed set from WP input (converted m/s -> cm/s)
        v_up_cms  = 250.0  # 2.5 m/s climb rate
        v_dn_cms  = 150.0  # 1.5 m/s descent rate
        h_spd_cms = max(speed * 100.0, 100.0)  # Convert speed to cm/s (minimum 1.0 m/s)
        try:
            if self.param_set_cli.wait_for_service(timeout_sec=0.3):
                ...
                self.get_logger().info(
                    ...
                    f"CLIMB: {v_up_cms/100:.1f}m/s | "
                    f"DESCENT: {v_dn_cms/100:.1f}m/s"
                )
        except Exception as e:
            self.get_logger().error(f'[GCS] Failed to set speed params: {e}')

        socketio.emit('krti_wp_update', {'wp': wp.get('wp'), 'name': wp.get('name'), 'type': wp_type})
        ...
        if lat != 0.0 and lng != 0.0 and self.ref_lat is not None and self.ref_lon is not None and self.ref_lat != 0.0:
            ...
            x = dist_h * math.sin(bearing_h)   # absolute ENU East from Home
            y = dist_h * math.cos(bearing_h)   # absolute ENU North from Home
        elif 'local_x' in wp and 'local_y' in wp:
            ...
            x = self.mission_origin_x + float(wp['local_x'])
            y = self.mission_origin_y + float(wp['local_y'])
        else:
            cur_lat = current_telemetry.get('lat', 0.0) or 0.0
            cur_lng = current_telemetry.get('lng', 0.0) or 0.0
            dist_h, bearing_h = haversine_distance_and_bearing(cur_lat, cur_lng, lat, lng)
            x = self.local_x + dist_h * math.sin(bearing_h)
            y = self.local_y + dist_h * math.cos(bearing_h)

        self.final_target_position = [x, y, z]

        if self.current_auto_wp_index > 0 and self.prev_target_position is not None:
        ...
        dy = y - start_y   # ENU North diff
        dx = x - start_x   # ENU East diff
        dist = math.sqrt(dx*dx + dy*dy)
        self.expected_leg_time = max((dist / max(speed, 1.0)) + 6.0, 8.0)

        # Set target_yaw to explicit WP target heading if present, else transit bearing atan2(dy, dx)
        ...
        # Phase 2 yaw: rotate to transit bearing BEFORE flying the leg
        ...
        # Check if this WP requires traffic light permission from RasPi
        if wp.get('require_permission', False):
            self.waiting_for_permission = True
            self.get_logger().info(f"[GCS] WP {self.current_auto_wp_index} requires RasPi traffic light permission.")

        current_telemetry['current_wp_index'] = self.current_auto_wp_index + 1

        self.get_logger().info(
            f"[GCS] WP{wp.get('wp')} ({wp_type.upper()}) → "
            f"ENU target: ({x:.2f}m E, {y:.2f}m N, {z:.2f}m U) | "
            f"Speed: {speed:.1f}m/s"
        )

        # Arm + takeoff on first WP
...
def handle_manual_action(data):
    ...
    if isinstance(data, dict):
        ...
        alt = max(1.0, min(50.0, float(data.get('alt', 2.5))))
        speed = max(0.5, min(10.0, float(data.get('speed', 4.0))))
    else:
        action_name = data
        alt = 2.5
        speed = 4.0
    ...
    if action_name == 'arm':
        ...
        if ros_node_instance.local_z > 0.5:
            ...
            ros_node_instance.target_yaw = ros_node_instance.current_yaw_rad  # ENU yaw from local_pos
            ...
            socketio.emit('upload_status', {'status': 'success', 'message': f'Go-around: Climbing back to {alt:.1f}m'})
        else:
            ...
            ros_node_instance.guided_active = True
            ros_node_instance.set_mode("GUIDED")
            socketio.emit('upload_status', {'status': 'success', 'message': f'Activating GUIDED mode & Taking off to {alt:.1f}m'})
...
def handle_manual_target(data):
    ...
    try:
        lat = float(data['lat'])
        lng = float(data['lng'])
        alt_rel = max(1.0, min(50.0, float(data['alt'])))
        speed = max(0.5, min(10.0, float(data.get('speed', 4.0))))
        ...
        current_lat = current_telemetry.get('lat', ros_node_instance.ref_lat)
        current_lng = current_telemetry.get('lng', ros_node_instance.ref_lon)
        if not current_lat or current_lat == 0.0: current_lat = ros_node_instance.ref_lat
        if not current_lng or current_lng == 0.0: current_lng = ros_node_instance.ref_lon

        dist_h, bearing_h = haversine_distance_and_bearing(current_lat, current_lng, lat, lng)
        x = ros_node_instance.local_x + dist_h * math.sin(bearing_h)
        y = ros_node_instance.local_y + dist_h * math.cos(bearing_h)
        z = alt_rel
...
@socketio.on('set_heading_offset')
def handle_set_heading_offset(data):
    """Koreksi heading kompas dari web UI. Offset dalam derajat (-180 s.d. +180)."""
    global current_telemetry
    offset = float(data.get('offset', 0.0))
    current_telemetry['heading_offset'] = offset
...
@socketio.on('pi_permission_granted')
def handle_pi_permission_granted(data):
    """RasPi confirms traffic light is GREEN → advance mission past current WP.
    RasPi emits: {'wp': <wp_index>}
    """
    if ros_node_instance is None: return
    wp_index = data.get('wp', -1)
    if ros_node_instance.waiting_for_permission and ros_node_instance.permission_wp_index == wp_index:
        ros_node_instance.get_logger().info(
            f"[GCS] \u2705 Permission GRANTED for WP {wp_index}. Advancing mission."
        )
        ros_node_instance.waiting_for_permission = False
        ros_node_instance._permission_request_t = None
        ros_node_instance.current_auto_wp_index += 1
        ros_node_instance.start_next_auto_waypoint()
        socketio.emit('status_msg', {
            'type': 'success',
            'text': f'[MISSION] \U0001f7e2 Green light! Advancing past WP {wp_index + 1}'
        })
    else:
        ros_node_instance.get_logger().warn(
            f"[GCS] Permission granted for WP {wp_index} but node waiting_for_permission={ros_node_instance.waiting_for_permission}, "
            f"permission_wp_index={ros_node_instance.permission_wp_index}. Ignoring stale grant."
        )

@socketio.on('pi_permission_denied')
def handle_pi_permission_denied(data):
    """RasPi reports traffic light is RED → drone holds at current WP.
    RasPi emits: {'wp': <wp_index>}
    The 15-second timeout in guided_timer_callback is the safety auto-advance fallback.
    """
    if ros_node_instance is None: return
    wp_index = data.get('wp', -1)
    ros_node_instance.get_logger().info(
        f"[GCS] \U0001f534 Permission DENIED for WP {wp_index}. Drone holding..."
    )
    socketio.emit('status_msg', {
        'type': 'warning',
        'text': f'[MISSION] \U0001f534 Red light \u2014 holding at WP {wp_index + 1}...'
    })
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            pass  # Already shut down by launch system SIGINT handler — safe to ignore
